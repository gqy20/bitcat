...
本文件为 OpenAI Codex (codex.openai.com) 提供项目指导。编码规范、测试规范、架构说明与 CLAUDE.md 保持同步。
...
cargo run -p xtask -- logs analyze <zip>   # 分析其他设备导出的诊断包（导出入口：托盘菜单「导出诊断包」）
```
...
| 产品 / 设置页 / 用户文案 | `docs/product/design-spec.md`（验收标准）、`docs/roadmap.md`（产品视角节）、`app/frontend/settings.html`、`app/src/settings.rs` |
...
- `core/src/agent.rs` — AI Agent 流式对话 + 16 个 Tool 定义 + rig stream status 派生
...
- `core/src/points.rs` — 积分、等级、连续活跃和成就薄片
- `core/src/gomoku_ai.rs` — 五子棋 AI 走法、候选事实表和结构化 commentary 校验
- `core/src/game_request.rs` — AI `start_game` 工具到 app ActionBus 的请求桥
