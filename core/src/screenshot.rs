...
fn default_max_width() -> u32 { 960 }
fn default_jpeg_quality() -> u8 { 80 }
fn default_interval_sec() -> u64 { 30 }
fn default_true() -> bool { true }
fn default_similarity_threshold() -> f64 { 0.95 }
fn default_min_width() -> u32 { 480 }
fn default_debug_resolutions() -> Vec<u32> { Vec::new() }
...
impl ScreenshotConfig {
    pub fn from_env() -> Self {
        ...
        if let Ok(v) = std::env::var("SCREENSHOT_MAX_WIDTH") {
            if let Ok(w) = v.parse::<u32>() { cfg.max_width = w; }
        }
        cfg
    }
    ...
    pub fn validate(&self) -> Result<(), String> {
        if self.jpeg_quality == 0 || self.jpeg_quality > 100 {
            return Err(format!("jpeg_quality 必须在 1-100 之间，当前: {}", self.jpeg_quality));
        }
        if self.max_width == 0 { return Err("max_width 不能为 0".into()); }
        Ok(())
    }
}
...
pub fn perceptual_hash(pixels: &[u8], w: u32, h: u32) -> u64 {
    ...
    for row in 0..h.min(8) {
        for col in 0..(w - 1).min(8) {
            ...
            if right > left { hash |= 1u64 << (row * 8 + col); }
        }
    }
    hash
}
...
pub fn similarity(a: u64, b: u64) -> f64 { 1.0 - (hamming_distance(a, b) as f64 / 64.0) }

pub fn is_similar(a: u64, b: u64, threshold: f64) -> bool { similarity(a, b) >= threshold }
...
pub fn stitch_horizontal(frames: &[&CapturedFrame]) -> CapturedFrame {
    if frames.is_empty() { return CapturedFrame { pixels: vec![], width: 0, height: 0 }; }
    ...
    for frame in frames {
        for y in 0..frame.height {
            ...
            pixels[dst_start..dst_start + src_end - src_start].copy_from_slice(&frame.pixels[src_start..src_end]);
        }
        x_offset += frame.width;
    }
    CapturedFrame { pixels, width: total_width, height: max_height }
}

// ---- Resize + JPEG 编码 ----

pub fn resize_bgra(bgra: &[u8], w: u32, h: u32, max_width: u32) -> Result<(Vec<u8>, u32, u32), String> {
    let expected = (w * h * 4) as usize;
    if bgra.len() < expected { return Err(format!("BGRA buffer too small: expected {} bytes, got {}", expected, bgra.len())); }
    let (out_w, out_h) = if w <= max_width { (w, h) } else {
        let scale = max_width as f64 / w as f64; (max_width, (h as f64 * scale).round() as u32)
    };
    let mut rgba = Vec::with_capacity((w * h * 4) as usize);
    for chunk in bgra[..expected].chunks_exact(4) { rgba.push(chunk[2]); rgba.push(chunk[1]); rgba.push(chunk[0]); rgba.push(chunk[3]); }
    use image::{ImageBuffer, RgbaImage, imageops};
    let img: RgbaImage = ImageBuffer::from_raw(w, h, rgba).ok_or_else(|| "无法创建图像缓冲区".to_string())?;
    ...
    for pixel in resized.pixels() { rgb.push(pixel[0]); rgb.push(pixel[1]); rgb.push(pixel[2]); }
    Ok((rgb, out_w, out_h))
}
...
pub fn encode_jpeg(rgb: &[u8], w: u32, h: u32, quality: u8) -> Result<Vec<u8>, String> {
    ...
    use std::io::Cursor;
    let img: RgbImage = ImageBuffer::from_raw(w, h, rgb.to_vec()).ok_or_else(|| "无法创建 RGB 图像缓冲区".to_string())?;
    let mut buf = Cursor::new(Vec::new());
    let encoder = JpegEncoder::new_with_quality(&mut buf, quality);
    img.write_with_encoder(encoder).map_err(|e| format!("JPEG 编码失败: {e}"))?;
    Ok(buf.into_inner())
}
...
pub fn save_screenshot(jpeg_bytes: &[u8], record: &ScreenshotRecord) -> Result<std::path::PathBuf, String> {
...
pub fn save_analysis_json(dir: &std::path::Path, prefix: &str, suffix: &str, record: &ScreenshotRecord) -> Result<(), String> {
    let json_path = dir.join(format!("{prefix}{suffix}_analysis.json"));
    let json = serde_json::to_string_pretty(record).map_err(|e| format!("序列化分析结果失败: {e}"))?;
...
pub fn list_recent_analyses(dir: &std::path::Path, count: u32) -> Vec<ScreenshotRecord> {
    ...
    for entry in entries.flatten() {
        ...
        if !name.ends_with("_analysis.json") { continue; }
...
#[allow(dead_code)]
pub fn list_recent_analyses_multi_day(base_dir: &std::path::Path, count: usize) -> Vec<(String, ScreenshotRecord)> {
    let Ok(entries) = std::fs::read_dir(base_dir) else { return Vec::new(); };
    let mut day_names: Vec<String> = entries.flatten().filter_map(|e| {
        let name = e.file_name().to_string_lossy().to_string();
        if e.path().is_dir() && name.len() == 10 && name.contains('-') { Some(name) } else { None }
    }).collect();
    ...
    for day in &day_names {
        ...
        for record in list_recent_analyses(&day_dir, needed as u32) { results.push((day.clone(), record)); }
    }
    results
}
...
pub fn build_recent_analyses_context(count: usize, max_chars: usize) -> String {
    let base = match screenshot_base_dir() { Ok(b) => b, Err(_) => return String::new() };
    build_recent_analyses_context_with_base(count, max_chars, &base)
}

pub fn build_recent_analyses_context_with_base(count: usize, max_chars: usize, base_dir: &std::path::Path) -> String {
    ...
    for (i, (_day, record)) in records.iter().rev().enumerate() {
        ...
        if result.chars().count() + line.chars().count() + footer_chars > max_chars { break; }
...
pub fn cleanup_old_screenshots(keep_days: u64) -> Result<u32, String> {
    let base = screenshot_base_dir()?; if !base.exists() { return Ok(0); }
    ...
    for entry in entries.flatten() {
        ...
        if name_str < cutoff_str && std::fs::remove_dir_all(entry.path()).is_ok() { removed += 1; }
    }
    Ok(removed)
}
