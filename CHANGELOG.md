...
- **飞机守护战玩法**：将原守护战改为飞行射击玩法，支持方向持续移动、A / Space 发射、X/Y 技能、L1 防护、HP 和目标进度 HUD。
- **小游戏爽感反馈**：Snake / Memory / Catch / Battle 增加 combo、弹字、粒子、爆炸、屏幕震动、BOOST 状态和更明确的成功/失败反馈。
- **小游戏计分体系**：翻牌记录 flips / moves / misses 并按通关效率追加奖励；接食物连续接住加分、漏掉扣分；飞机战连续击杀加分、受伤扣分；Snake 加速吃到食物得分更高。
- **Cat v2 资源包**：内置 `cat` 升级为 192×208、64 帧、高分辨率 v2 companion 资源包，并新增可重复生成脚本。
- **额外角色资源包**：新增 `byte-bun`、`mossbot`、`moonbit`、`sparkle` 等内置角色资源包，资源 catalog 覆盖更多可选宠物风格。
- **Steam 本地 SDK 探测**：新增本地 Steam SDK / DLC 策略相关探测与规划文档，为后续能力解锁和商店集成做准备。

### 修复

- 修复飞机守护战手柄方向只触发一次、长按无法持续左右移动的问题；方向松开时现在会发送停止输入。
- 修复接食物和飞机战横向区域过宽导致移动跟不上的问题，二者都收敛为居中半宽赛道/航道。
- 修复飞机战旧圆圈背景和技能块残留，改为更贴近飞行射击的航道背景、飞机、敌人、子弹和 HUD。
- 修复 Catch 速度偏快的问题，默认速度和掉落节奏更适合手柄操作。
- 修复 Agent Watch 远程安装、任务卡和 session context 的若干展示与安全检查细节。

### 变更

- 弹出面板默认收敛为 2×2 小游戏面板，只保留毛线球大作战、翻牌配对、接食物、飞机守护战四个入口。
- Snake 默认网格从 30×20 提高到 48×32，胜利长度和速度同步重调，渲染改为圆润连续身体、曲线转弯、强化边框和高亮食物。
- Snake 前 20 个食物优先刷在中间区域，降低开局贴边和长距离折返压力；按住 A / Space / Enter 可临时加速。
- 飞机守护战目标从 20 提高到 50，漏怪和碰撞会明确扣 HP、扣分并打断 combo。
- 接食物改为半宽赛道、篮子造型和 combo 反馈；翻牌卡片改为圆角卡牌、符号图案和匹配/失败反馈。
- 宠物资源默认与 catalog 继续向 WebP spritesheet + manifest v2 收敛，旧 PNG sheet 路径移除。

### 工具链

- 更新 core / app / xtask / Tauri 版本号到 0.1.6。
- 新增和更新小游戏、sprite loader、pet fixture、Agent Watch、Steam SDK 探测相关测试。
- 新增 `generate-cat-pack.cjs` 和额外 pet pack 生成脚本，便于稳定复现内置资源。

### 文档

- 更新 README 的发布 tag 示例、弹出面板、迷你游戏和宠物资源包说明，反映 0.1.6 的实际默认体验。
- 补充 top-down RPG AI、能力解锁、Steam DLC 策略等规划文档。
- 更新 Agent Watch / Remote Agent Watch 文档，覆盖远程安装、安全检查和任务卡细节。

---

## [0.1.5] - 2026-05-17

0.1.5 聚焦 Remote Agent Watch、宠物 v2 资源包和语义短动作反馈，强化远程看管与桌宠表现。

### 新增

- **Remote Agent Watch viewer**：新增远程 Agent 看管 viewer，远程 Mac/Linux 可通过 `5342` 上报 hook envelope，并通过 `5344 /watch` 查看只读状态。
- **远程一键安装入口**：设置页 Agent 看管区域新增远程安装命令、watch URL、远程设备列表和复制按钮，远程机器可直接从 `5344 /remote-install.sh` 拉取安装脚本，不需要检出本仓库。
- **远程 endpoint 发现**：远程安装命令生成已拆到 `app/src/remote_endpoint.rs`，支持 LAN、Tailscale/tailnet、VPN 和其他可达地址候选，并通过 `--hosts` 支持多地址回传重试。
...
- 修复远程看管 URL 可能错误选择 `198.18/15` 虚拟/benchmark 网段的问题；现在会按 endpoint 类型排序，并把 tailnet/CGNAT `100.64/10` 纳入候选。
...
- Agent Watch 设置页默认脱敏展示远程 endpoint 标签，复制安装命令和 watch URL 时才包含完整地址。
...
- 远程看管 endpoint 选择逻辑增加 Windows `ipconfig` 解析、tailnet/CGNAT 排序、Tailscale adapter hint 和多地址安装命令测试。
...
0.1.4 聚焦 Agent Watch 接入、小游戏模式扩展、宠物外部资产加载和发布工具链修复，增强桌宠协作与可玩性。
...
0.1.3 聚焦 AI 语义事件、长期记忆、音乐响应舞动、守护战斗模式和宠物动画表现，提升桌宠的智能反馈能力。
...
0.1.2 聚焦面板配置化、多屏观察性能和桌宠位置/设置稳定性，改善日常桌面使用体验。
