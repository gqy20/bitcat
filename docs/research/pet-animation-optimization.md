...
> **日期**: 2026-05-13
> **当前状态**: 本文是研究基线。当前本地代码已落地非均匀帧时长、repeat + fallback、语义事件/通知生命周期，以及 v2-only 宠物资源包。历史执行计划见 [`docs/plan/archive/pet-animation-visual-roadmap.md`](../plan/archive/pet-animation-visual-roadmap.md)；后续资源包发布策略见 [`docs/plan/pet-asset-packaging.md`](../plan/pet-asset-packaging.md)。
>
> ## 项目位置关系
>
> ```
...
| # | 优化方向 | 改动量 | 效果 | 风险 | 优先级 |
|---|---------|--------|------|------|--------|
| 1 | **非均匀帧时长** | ~~小~~ ✅ **已完成 (2026-05-13)** | Idle 立刻更有生命力 | 低（向后兼容，可渐进迁移） | **P0 — 已落地** |
| 2 | **三遍+回落** | ~~小~~ ✅ **已完成 (2026-05-13)** | 状态过渡更自然 | 低（纯 additive） | **P0 — 已落地** |
...
```
Phase 1 ✅ (已完成):  #1 + #2 — 改造 STATE_CONFIG 和 update()
                     │  idle 呼吸更自然（1500/120/200/120/1800ms）
                     │  瞬态 repeat+fallback 替代硬编码超时
                     │  同时改了 JS + Rust 两套状态机（镜像同步）
                     ▼
