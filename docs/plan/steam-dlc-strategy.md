...
| **记忆系统** | 短期记忆（20 条）+ 长期记忆（200 条） | 对话质量的基础，200 条够日常使用 |
| **用户画像** | `user.yml` 完整功能 | 对话个性化的基础 |
| **舞蹈系统** | 5 个基础动作 + 2 个预设 + AI 编舞 | AI 编舞是核心差异化，不能锁 |
| **小游戏** | Snake 贪吃蛇 | 一个游戏足够证明"桌宠能玩" |
...
| 游戏 | 引擎类 | 代码行数 | 特色 |
|------|--------|---------|------|
| Memory 记忆翻牌 | `MemoryEngine` | ~120 行 | 4×4/6×6 卡牌、匹配配对 |
| Catch 接物 | `CatchEngine` | ~115 行 | 物品下落、限时接取 |
| Battle 射击对战 | `BattleEngine` | ~670 行 | 技能 CD、护盾、多敌人波次、浮动伤害 |
...
| 风险 | 影响 | 缓解 |
|------|------|------|
| 免费版内容太少，用户觉得"没什么可玩" | 留存差 | 免费版必须包含完整 AI 对话 + 2 宠物 + Snake + 编舞，核心体验不缩水 |
