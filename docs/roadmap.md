...
| 截图观察 + 摄像头观察 + Vision API 分析 | 截图当前默认启用（F3/D1 计划改为首次由用户亲手开启）；摄像头观察默认关闭，可在设置页开启 |
| 滚动窗口记忆系统 | 已有 |
| YML 配置热加载 | 已有 |
...
宠物资源线已收敛（15 个 `cat-*` 品种），工程流水线已就位（见基线表末行）。当前最大风险**不是功能不够，而是产品不可理解**：设置页按代码模块组织、高权限默认值未经恐慌测试。下一步按产品主线推进：

1. **F1 设置页重组（4+1）**：把 11 个 tab 收敛为四个“用户疑问”分区 + 折叠的专家模式，用户区控件 77 → ~20。纯前端 IA 调整，性价比最高的一步。
2. **F2 功能使用审计**：读本地 JSONL（points/tool/token/reminder events）统计真实使用频次，输出“砍 / 藏 / 留”清单。项目自带埋点，不引入 analytics 服务。
3. **D1 权限向导 = 信任叙事**：从“合规任务”升级为“新用户第一次信任建立”，3 步故事（它是谁 → 它会什么 / 不会什么 → 随时可收回什么）；截图观察改为首次由用户亲手开启。
4. **D4 Invasion 收口**：把 `Invasion` 从 MVP 打磨成 Demo 核心切片，完成真实窗口触发回归、敌人节奏/目标反馈、手柄/键盘路径一致性和专属积分事件。
5. **A2/B7 分数与成长闭环**：补分数 JSONL、Invasion 专属 points/achievement、发布版能力开关，为 Steam Achievements/Cloud 打本地事实基础。
6. **D2/D3 Steam 平台与合规**：SteamPipe、成就/统计、云存档排除敏感目录；D3 的数据导出/删除直接落在 F1 的“它记住了我什么”分区，一次实现两处收益。
...
│  先修理解与信任（F1-F3），再补核心玩法、权限合规与 Steam 集成           │
└─────────────────────────────────────────────────────────────┘
```
...
| 阶段 | 目标 | 必须完成 | 不做/后置 |
|------|------|----------|-----------|
| **Steam Demo / Playtest** | 验证安装、启动、权限说明、AI 对话、小游戏窗口和 Steam 链路 | F1 设置页 4+1 重组、F2 审计执行、D1 三步信任向导（截图观察改手动开启）、干净机器 smoke test、AI 内容披露草案、SteamPipe 上传脚本、最小 Steam init 诊断、`Invasion` 可反复游玩的核心玩法切片 | DLC、Workshop、复杂社区功能、完整 RPG |
...
1. **首次启动权限向导**：按 D1 的 3 步信任叙事实现（它是谁 → 会什么 / 不会什么 → 随时可收回什么），明确 AI 对话、截图观察、摄像头观察、文件读取、shell、剪贴板、前台窗口控制分别会做什么；**截图观察不再默认启用，改为向导中由用户亲手开启**（2026-09-09 调整：原“可默认启用”未通过恐慌测试）；摄像头继续默认关闭，shell/文件/剪贴板保持显式确认或安全边界。
...
2026-05-13 │  A1 舞蹈系统                         │
           ...
           └─────────────────────────────────────┘
                  ↓
短期        ┌─────────────────────────────────────┐
1-3天      │  F1 设置页重组 4+1（77→~20 控件）       │  ← 性价比最高：先让产品可理解
           │  F2 功能使用审计（本地 JSONL）          │  ← 用真实数据定砍/藏/留
           │  F3+D1 权限向导 = 3 步信任叙事          │  ← 截图观察改用户亲手开启 + smoke 清单
...
| **F1** | 设置页 4+1 重组（用户区/专家区拆分） | ~400-700 行（纯前端为主，Rust 命令层基本复用） | 0 | P0，1-2 天 |
| **F2** | 功能使用审计：读本地 JSONL 出砍/藏/留清单 | ~100-200 行（分析工具，可放 xtask） | 0 | P0，0.5 天 |
| **F3** | 默认值恐慌测试：截图观察改首次手动开启、panel 游戏入口收敛 | ~100 行 | 0 | P0，0.5 天（与 D1 合并实现） |
...
**当前可玩 Demo 的基础设施已超过原 MVP 预期；下一阶段最短路径是 F1/F2 产品可理解性（设置重组 + 使用审计）→ D1 权限向导信任叙事 → D4 Invasion 收口 → A2/B7 分数、成长和权限闭环 → D2/D3 Steam 平台与合规收束。目标是先让 BitCat 成为用户能理解、能信任、可审查、可反复游玩的 Steam Demo，再推进 Early Access。**
...
| 领域 | 当前边界 | 打磨目标 |
|------|----------|----------|
| 设置页 IA | 11 tab / 77 控件按代码模块组织，连作者都难以导航 | F1 重组为四个“用户疑问”分区 + 折叠专家模式；F2 审计后执行砍/藏/留 |
