...
> **产品主线（2026-09-09）**：用户价值不等于功能数量。BitCat 的护城河是“高权限 AI 伴侣里最让用户放心的那一个”——按陪伴/互动/照料三个时刻组织产品，用透明与可控建立信任；所有 Track 的排期都服务于这条主线，判断基准见下方「产品视角」节与 Track F。
...
| 截图观察 + 摄像头观察 + Vision API 分析 | 截图观察已改为默认关闭，首次启动由 D1 三步信任向导让用户亲手开启（F3 已落地，2026-09-11）；摄像头观察默认关闭 |
| 滚动窗口记忆系统 | 已有 |
| YML 配置热加载 | 已有 |
...
## 产品视角：三个时刻与四个疑问（2026-09-09）

> 本节是后续排期的判断基准，先于所有 Track：**用户价值不等于功能数量，理解成本是真实成本。**
> 结论一句话：BitCat 的护城河不是功能数量，而是“高权限 AI 伴侣里最让用户放心的那一个”。
>
> **2026-09-14 更新**：F1/F3+D1 已落地（设置页现为 4+1 结构，截图观察默认关闭、首次由向导亲手开启）；下文 2026-09-09 的诊断与“现状”列保留为当时的决策依据。
...
| **照料** | <1%，但决定信任与留存 | 四个疑问（见下） | 三个按钮内找到答案 | ❌ 11 个 tab 的认知瘫痪 |

**照料时刻的四个真实疑问**——设置页唯一应该围绕的东西：

1. “它在偷看我的屏幕吗？”（截图 / 摄像头 / shell / 剪贴板 / 热键权限）
2. “它记住了我什么？”（长期记忆内容，能否查看 / 导出 / 删除）
3. “这个月花了多少钱？”（token 成本与去向）
4. “它刚才干了什么？”（执行过的命令、弹通知的原因）

### 现状诊断（2026-09-09 实测）

- 设置页 **11 个 tab / 77 个可交互控件**，tab 名是 `memory / prompts / agent-watch / usage` 这类**代码模块名**，不是用户任务名。
- `prompts` tab（编辑提示词 YAML）、宠物事件决策日志、hook doctor、token 链路占比是**开发者诊断台**，不是用户设置——两个产品被缝在了一起，这是“连作者都难以导航”的根源。
- **截图观察默认开启**（每 30s 截全屏发 Vision API）：工程上是核心能力，用户视角是新用户 T+5min 的信任危机点。

### 恐慌测试（Panic Test）

每个新默认值、新入口合并前必须过一遍新用户时间线：

```text
T+10s   托盘出现猫                → 可爱 ✅
T+60s   对话要 API Key             → 学习成本，可接受
T+5min  气泡：“我看到你在写代码”    → 恐慌：“它一直在截我的屏？！”
T+1day  打开设置：11 个 tab         → 认知瘫痪 → 卸载
```

规则：**每个能力 = 一句不含术语的解释 + 一个显式开关 + 默认值倾向用户而非功能展示。**
越强的能力，越应该由用户第一次亲手打开。该规则适用于全部 Track 的新功能。
...
│  └──────────┘  └──────────┘  └──────────┘  └──────────┘   │
│                                                             │
│  主线：F 产品可理解性 ✓ → Steam Demo → Early Access → 正式版          │
│  理解与信任已修（F1-F3 ✓），再补核心玩法收口、Steam 集成与合规          │
└─────────────────────────────────────────────────────────────┘
```
...
### A4. 上班金币掉落（薪资陪伴，2026-09-14 构想）

定位：**陪伴时刻**的环境级表演——宠物陪用户上班，把"时间在流逝、钱在挣"变成可见的桌面节奏。非 AI 生成内容，复用 A1/A2 已有表演基础设施（game 透明窗口 / particles 粒子 / `PetEvent::React` 情绪）。

核心账本——从墙钟推导，不做 tick 累加（电脑睡眠/重启后数字依然准确）：

- 配置三项：月薪、上下班时间、每月工作日数（默认 21.75；第一版不做节假日历和午休扣除）。
- `core/src/earnings.rs` 纯函数 `earned_between(config, start, now)`，rstest + proptest 覆盖跨零点/月末/非工作日。
- 掉落节奏两层：**常驻小雨**（每攒 ¥0.1 掉一枚，3000 月薪档约 20 秒一枚，分钟级不打扰）+ **下班结算金币喷泉**（整个功能唯一必须精彩的瞬间，联动 `React` Happy）。

产品红线（薪水是高敏感数据）：

- 默认关闭，归入 F1「④ 它怎么陪着我」分区；无 F2 使用数据的新功能默认先在专家模式灰度。
- 设置页配一句人话解释（"只用来在你上班时掉金币，不会告诉任何人"）；只存本地 `app_settings.json`。
- 第一版不接 points 商店/喂食闭环，先验证掉落本身的陪伴感；不反映加班/早退，稳定压倒精确。

预估代码量约 1200 行（测试约占 1/4）：阶段一 MVP ~820 行（core earnings ~400 / app 调度+配置 ~210 / 前端金币动画+设置项 ~210），阶段二金币雨窗口+下班喷泉 ~410 行。三个最贵的部件项目已有：透明置顶窗口（game.rs）、粒子模式（particles.js）、轮询调度模板（reminder_scheduler.rs，实测 93 行）。实现顺序：core 纯逻辑+测试 → app 轮询 emit → particles 金币动画 → 设置项 → 阶段二再评估独立金币雨 def。

排期：**P3 待排期**——优先给 D4 / A2/B7 / D2/D3 等 Demo 闸门让路。F1 已落地，届时配置 UI 直接进「④ 它怎么陪着我」分区，并遵守用户区 zero-sum（加一项须移一项）。
...
## Track F: 产品可理解性与信任（2026-09-09 新增）

> 判断基准见上方「产品视角」节。这条线**不做新功能**，只做三件事：让用户理解、让用户放心、砍掉让用户困惑的东西。优先级高于一切新内容扩展（A3/C/E 的后续项都要让路）。

### F1. 设置页重组（4+1 信息架构）

> ✅ **已落地**（2026-09-09，提交 `c9e78ef`；`1571b23` / `f543702` / `bc33ae2` / `8869212` 完成专家模式分组与视觉交互打磨）。以下为重组方案，保留作 4+1 结构的验收基准。

把“用户设置”和“开发者诊断台”拆成两个产品。新 IA 直接按照料时刻的四个疑问组织：

```text
① 它能做什么 —— 权限总览仪表（设置页首屏）
   截图观察 ● │ 摄像头观察 ○ │ Shell/文件/剪贴板 受限 │ 一键“全部收权”
② 它记住了我什么 —— 记忆按人话展示 + 导出/删除
   “它知道：你在做 bitcat 项目、养猫、习惯晚上工作”  [查看] [删除]
③ 它花了多少钱 —— 今日 / 本月成本与去向（对话 / 观察 / 摘要）
④ 它怎么陪着我 —— 品种 / 舞蹈 / 游戏 / 提醒（唯一“开心”的分区）
⑤ 专家模式（默认折叠，单次点击进入）
   prompts YAML 编辑、宠物事件决策日志、hook doctor、token 链路明细、日志路径
```

迁移原则：

- 现有 11 个 tab 的每一项必须进映射表：**用户区 / 专家区 / 删除**，不允许“暂时都保留”。
- 用户区目标 ~20 个控件；`prompts` tab 从用户界面整体移除（编辑提示词是开发工作流）。
- overview 首屏四张卡片改为直接回答四个疑问，而不是模块统计。
- 实现上以前端为主（settings.html / settings.js / settings.css），Rust 命令层基本复用；预计 1-2 个开发日。

### F2. 功能使用审计

> 状态：审计工具已落地（`e17b4ab`，`cargo run -p xtask -- audit-usage --days N`）；panel 游戏库收敛已执行。第一轮结论见下。

项目自带本地埋点，无需引入 analytics 服务：

| 数据源 | 回答的问题 |
|--------|-----------|
| `~/.bitcat/logs/points_events.jsonl` | 舞蹈 / 游戏 / 提醒 / 夸奖的真实频次 → 玩法留存 |
| `~/.bitcat/logs/tool_events.jsonl` | 16 个工具里 AI 实际调用了哪几个 → 工具表面积 |
| `~/.bitcat/logs/token_usage.jsonl` | 成本花在对话 / 视觉 / 摘要哪条链路 → 成本结构 |
| `~/.bitcat/logs/reminder_events.jsonl` | 提醒完成率 vs 无视率 → 提醒的真实价值 |

产出一份“砍 / 藏 / 留”清单并执行。待验证的预判：8 个游戏用户 90% 时间只在 1-2 个——Demo 主推 Invasion，其余收进“猫的游戏库”单入口；音乐舞动 / TTS / 摄像头观察属长尾，进专家模式；**每砍掉一个功能，都在给剩下的功能让出认知带宽。**

#### 第一轮审计结论（2026-09-11，样本：作者本人 3 天）

数据：`cargo run -p xtask -- audit-usage --days 3`。

- 提醒是唯一高频真实使用（ReminderCompleted 42 次/3 天，喝水提醒），**留并继续强化**（B6 是价值锚点）。
- 夸奖宠物 11 次，低成本高情感，留。
- 游戏 / 舞蹈 / 观察类事件全零（样本仅作者、开发期非常驻使用，不能强断言），与预判方向一致：**panel 8 游戏收敛为「桌面保卫战主推 + 游戏库二级入口」**（已执行，`group: library` 机制）。
- `tool_events` / `reminder_events` 无数据文件：AI 工具与提醒创建从未在真实使用中触发（提醒由测试 fixture 直接写 store），留待真实使用 2 周后复审。
- `token_usage` 全为 test-model 测试数据，审计无意义；等真实链路积累。
...
对现有全部默认值与入口过一遍恐慌测试（规则见产品视角节），首批候选：

- 截图观察默认开启 → 改为首次启动由用户亲手开启（与 D1 向导合并实现）。
- panel 8 个游戏并排 → 收敛为“主推 1 个 + 游戏库入口”。
- 把规则写进贡献约定：新能力落地必须同时给出“一句人话解释 + 显式开关 + 恐慌测试结论”，否则不合并。

### F4. 透明即卖点（与 D5 联动）

商店页与设置页说同一种语言：卖点段落直接复用四个疑问的答案——“你随时能看到它看到了什么、记住了什么、花了多少钱、做过什么”。**透明本身是卖点**，优于罗列 16 工具 / 8 游戏——这是高权限 AI 伴侣品类里最稀缺的承诺。
...
| 阶段 | 目标 | 必须完成 | 不做/后置 |
|------|------|----------|-----------|
| **Steam Demo / Playtest** | 验证安装、启动、权限说明、AI 对话、小游戏窗口和 Steam 链路 | F1 设置页 4+1 重组 ✅、F2 审计执行 ✅（第一轮）、D1 三步信任向导 ✅；剩：干净机器 smoke test、AI 内容披露草案、SteamPipe 上传脚本、最小 Steam init 诊断、`Invasion` 可反复游玩的核心玩法切片 | DLC、Workshop、复杂社区功能、完整 RPG |
...
1. **首次启动权限向导**：✅ 已落地（2026-09-11，`c8c6ee5`）。按 D1 的 3 步信任叙事实现（它是谁 → 会什么 / 不会什么 → 随时可收回什么），明确 AI 对话、截图观察、摄像头观察、文件读取、shell、剪贴板、前台窗口控制分别会做什么；**截图观察不再默认启用，改为向导中由用户亲手开启**（2026-09-09 调整：原“可默认启用”未通过恐慌测试）；摄像头继续默认关闭，shell/文件/剪贴板保持显式确认或安全边界。
...
商店页不要把 BitCat 写成通用 AI 助手或完整 RPG。更稳妥的品类是：桌面宠物、AI companion、casual、utility-light game、cozy/productivity companion。正式卖点应围绕“桌宠陪伴 + 可玩互动 + 本地成长 + 玩家可控权限”；具体文案直接复用 F1 四个疑问的答案（见 F4：透明即卖点），**不罗列工具/游戏数量清单**。
...
2026-05-13 │  A1 舞蹈系统                         │
           ...
           └─────────────────────────────────────┘
                  ↓
已完成      ┌─────────────────────────────────────┐
2026-09     │  F1 设置页重组 4+1（77→~20 控件）      │
           │  F2 使用审计第一轮 + panel 游戏库收敛   │
           │  F3+D1 默认收权 + 三步信任向导          │
           │  常驻资源/IPC 性能治理收尾（P4/L4）     │
           └─────────────────────────────────────┘
                  ↓
短期        ┌─────────────────────────────────────┐
...
F1(设置重组) ──→ D3(导出/删除) 数据权利落位，一次实现两处收益
F2(使用审计) ──→ A3/D4 先审计再扩功能，用数据定砍藏留
F3(恐慌测试) ──→ D1(权限向导) 向导即首次信任建立，截图改手动开启
```
...
**F 线（设置重组 / 使用审计 / 默认收权 + 信任向导）已闭合，产品可理解性主线初步达成；下一阶段最短路径是 D4 Invasion 真机收口 → A2/B7 分数、成长和权限闭环 → D2/D3 Steam 平台与合规收束 → 可审查的 Steam Demo，再推进 Early Access。**
...
| 领域 | 当前边界 | 打磨目标 |
|------|----------|----------|
| 设置页 IA | F1 已落地：4+1 结构（四疑问分区 + 折叠专家模式） | 维持用户区控件 ≤20 的 zero-sum；F2 第二轮复审后继续砍/藏/留；新设置项一律按 4+1 归类 |
